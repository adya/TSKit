...
public extension Date {
    ...
    func isLater(than date: Date, by days: Int) -> Bool {
        Calendar.current.dateComponents([.day], from: self, to: date).day ?? 0 >= days
    }
}
...
@available(iOS 8.0, *)
public func !=(date1 : Date, date2 : Date) -> Bool {
    date1.compare(date2) != .orderedSame
}
...
private func compareDates(_ date1 : Date, date2 : Date, granularity : Calendar.Component) -> ComparisonResult {
    Calendar.current.compare(date1, to: date2, toGranularity: granularity)
}

/// Checks whether the first date is greater than the second date by comparing only their date components (e.g. Year, Month, Day)
@available(iOS 8.0, *)
public func >!(date1 : Date, date2 : Date) -> Bool {
    compareDates(date1, date2: date2, granularity: .day) == .orderedDescending
}
...
@available(iOS 8.0, *)
public func <!(date1 : Date, date2 : Date) -> Bool {
    compareDates(date1, date2: date2, granularity: .day) == .orderedAscending
}
...
@available(iOS 8.0, *)
public func ==!(date1 : Date, date2 : Date) -> Bool {
    compareDates(date1, date2: date2, granularity: .day) == .orderedSame
}
